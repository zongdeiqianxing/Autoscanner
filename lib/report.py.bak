...
class Report:
    ...
    def html_report_single(self, target):
        with sqlite3.connect(os.path.join(main_path, 'scanned_info.db')) as conn:
            ...
            sql = "SELECT count(*) from scanned_info where batch_num = {now_time} and domain LIKE '%{domain}%';".format(now_time=now_time, domain=target.data['domain'])
            for count in conn.execute(sql):
                if count[0] < 2:
                    # host扫描报告
                    sql = '''select * from host_info where batch_num = {now_time} and domain = '{domain}';'''.format(now_time=now_time, domain=target.data['domain'])
                    for _ in parse(conn.execute(sql)):
                        tag += _

            # web扫描报告
            sql = '''SELECT * from scanned_info where batch_num = {now_time} and domain LIKE '%{domain}%';'''.format(now_time=now_time, domain=target.data['domain'])
            for _ in parse(conn.execute(sql)):
                tag += _
