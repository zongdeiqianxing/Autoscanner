...
class Oneforall(Tools):
    def filter_log(self):
        try:
            if self.logfile:
                self.logfile = self.logfile.replace('www.', '')
...
class Nslookup(Tools):
    ...
    def filter_log(self):
        cdns = ['cdn', 'kunlun', 'bsclink.cn', 'ccgslb.com.cn', 'dwion.com', 'dnsv1.com', 'wsdvs.com', 'wsglb0.com',
                'lxdns.com', 'chinacache.net.', 'ccgslb.com.cn',]
        for cdn in cdns:
            if cdn in self.run_log:
                logger.warning("{} may be is cdn, scan will be skipped")
                self.run_log += '可能存在cdn：{} ~ 将跳过masscan与nmap扫描'.format(cdn)
...
class Whatweb(Tools):
    def filter_log(self):
        if self.run_log:
            # 有些时候会报80端口无法访问错误
            if 'The plain HTTP request was sent to HTTPS port]' in self.run_log:
                self.cmd.replace('http', 'https')
                self.scan()
